...
def parse_number_words(tokens: List[str]) -> Optional[float]:
    """
    Parse simple English number phrases into a numeric value.
    Supports units, teens, tens, and multipliers like hundred/thousand.
    Example inputs (already lowercased, fillers removed):
      ["one", "hundred"] -> 100
    ...
    while idx < len(tokens):
        ...
        if word in _NUMWORD_FILLERS:
            idx += 1
            continue

        if word in _NUMWORD_UNITS:
            current += _NUMWORD_UNITS[word]
            idx += 1
            continue

        if word in _NUMWORD_TEENS:
            current += _NUMWORD_TEENS[word]
            idx += 1
            continue

        if word in _NUMWORD_TENS:
            value = _NUMWORD_TENS[word]
            # Check for trailing unit (twenty two -> 22)
            if idx + 1 < len(tokens) and tokens[idx + 1] in _NUMWORD_UNITS:
                value += _NUMWORD_UNITS[tokens[idx + 1]]
                idx += 1
            current += value
            idx += 1
            continue

        if word in _NUMWORD_MULTIPLIERS:
            multiplier = _NUMWORD_MULTIPLIERS[word]
            if current == 0:
                current = 1
            current *= multiplier
            total += current
...
def extract_numeric_answer(text: str, field_type: str) -> Tuple[bool, Optional[Any]]:
    """
    Extract a numeric value from a free-form text answer.

    field_type: "account", "amount", "otp", "pin", etc.
    Returns: (confident, value)
      - confident: True/False (whether we're confident enough to use this as the final value)
      - value: normalized value (digit string or float) or None if not confident.
    """
    if not text:
        return False, None
    ...
    digit_runs: List[str] = []
    current = ""
    numword_tokens: List[str] = []
    junk_tokens: List[str] = []
    for token in tokens:
        if token.isdigit():
            current += token
        else:
            ...
            if token in _FILLER_WORDS:
                continue
            if token in _NUMWORD_UNITS or token in _NUMWORD_TEENS or token in _NUMWORD_TENS or token in _NUMWORD_MULTIPLIERS or token in _NUMWORD_FILLERS:
                numword_tokens.append(token)
            else:
    ...
    junk_ratio = len(junk_tokens) / total_tokens if total_tokens else 0.0

    def digit_run_candidate(min_len: int = 1, max_len: int = 30) -> Optional[str]:
        if len(digit_runs) == 1:
            raw = digit_runs[0]
            if min_len <= len(raw) <= max_len and junk_ratio <= 0.6:
                return raw
        return None

    if field_type in {"account", "card", "loan"}:
        raw = digit_run_candidate(min_len=4, max_len=20)
        if raw:
            return True, raw
        return False, None

    if field_type in {"otp", "pin"}:
        raw = digit_run_candidate(min_len=4, max_len=6)
        if raw and len(raw) in {4, 6}:
            return True, raw
        return False, None

    if field_type == "amount":
        raw = digit_run_candidate(min_len=1, max_len=12)
        if raw:
            try:
                value = float(raw)
        ...
        if numword_tokens:
            filtered = [t for t in numword_tokens if t not in {"rupees", "dollars", "bucks"}]
            word_value = parse_number_words(filtered)
            if word_value is not None and 0 < word_value <= 1_000_000:
                return True, float(word_value)
...
def normalize_tool_input_numeric(tool_name: str, tool_input: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Normalize numeric fields in tool_input using extract_numeric_answer.
    """
    if not tool_input:
        return {}
    ...
    for field in account_fields:
        ...
        if isinstance(raw, str):
            confident, value = extract_numeric_answer(raw, "account")
            if confident and isinstance(value, str):
                normalized[field] = value

    if "amount" in normalized and isinstance(normalized["amount"], str):
        confident, value = extract_numeric_answer(normalized["amount"], "amount")
        if confident and value is not None:
            normalized["amount"] = value

    if "amount_raw" in normalized:
        confident, value = extract_numeric_answer(str(normalized["amount_raw"]), "amount")
        if confident and value is not None:
            normalized["amount"] = value
        normalized.pop("amount_raw", None)

    otp_fields = ["otp", "otp_code", "verification_code", "security_code", "pin"]
    for field in otp_fields:
        raw = normalized.get(field)
        if isinstance(raw, str):
            ftype = "pin" if "pin" in field else "otp"
            confident, value = extract_numeric_answer(raw, ftype)
            if confident and isinstance(value, str):
                normalized[field] = value
