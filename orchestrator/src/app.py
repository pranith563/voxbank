...
async def startup_event():
    ...
    try:
        ...
        logger.info("MCP client initialized (base=%s transport=%s)", app.state.mcp_client.base_url, getattr(app.state.mcp_client, "transport_mode", "unknown"))
    except Exception as e:
        logger.exception("MCP client initialize failed: %s", e)
    # Continue anyway - client might work with retry later
    ...
    # Attempt to discover & log tools right away (best-effort)
    try:
        # 1) Ask agent if it exposes get_tools()
        agent_tools = None
        agent = getattr(app.state, "agent", None)
        if agent:
            get_tools_fn = getattr(agent, "get_tools", None)
            if get_tools_fn:
                agent_tools = get_tools_fn()
                if asyncio.iscoroutine(agent_tools):
                    agent_tools = await agent_tools

        if agent_tools:
            logger.info("Agent-provided tools discovered at startup:")
            for t in agent_tools:
                logger.info(" - %s", t)
        else:
            # 2) Try calling MCP tool `list_tools` (curated tools expose this)
            try:
                mcp = getattr(app.state, "mcp_client", None)
                if mcp:
                    # prefer call_tool / call semantics; some clients name it call_tool or get_balance etc.
                    call_fn = getattr(mcp, "call_tool", None) or getattr(mcp, "call", None) or getattr(mcp, "execute", None)
                    if call_fn:
                        try:
                            # call list_tools with an empty payload (many implementations support this)
                            raw = await call_fn("list_tools", {}) if call_fn.__name__ == "call_tool" else await call_fn("list_tools", {})
                            # raw expected to be dict with 'tools' or similar
                            tools_list = None
                            if isinstance(raw, dict) and "tools" in raw:
                                ...
                                # if raw is nested, try to be flexible
                                tools_list = raw.get("tools") if isinstance(raw, dict) else None

                            if tools_list:
                                logger.info("MCP `list_tools` response at startup:")
                                for entry in tools_list:
                                    # entry may be dict or tuple-like
                                    if isinstance(entry, dict):
                                        name = entry.get("name") or entry.get("tool") or "<unnamed>"
                                        desc = entry.get("description", "")
                                        path = entry.get("path", "")
                                        logger.info(" - %s: %s (%s)", name, desc, path)
                                    elif isinstance(entry, (list, tuple)) and entry:
                                        logger.info(" - %s", entry)
                                    else:
                                        logger.info(" - %s", entry)
                            else:
                                logger.info("MCP `list_tools` returned nothing useful at startup: %s", raw)
                        except Exception as exc:
                            logger.debug("Calling MCP list_tools failed: %s", exc)
            except Exception:
                logger.debug("MCP tool listing failed during startup; continuing.")

    except Exception as e:
        logger.exception("Tool discovery during startup failed: %s", e)

    logger.info("Orchestrator started. Gemini model=%s MCP base=%s", gemini_model, getattr(app.state.mcp_client, "base_url", "unknown"))
    logger.info("=" * 80)
...
async def process_text(request: TextRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # If agent needs clarification (missing information)
...
async def text_respond(req: TextRequest):
    ...
    try:
        ...
        session = SESSIONS.setdefault(session_id, {"user_id": req.user_id, "history": [], "pending_action": None})
        session["history"].append({"role": "assistant", "text": response_text})
        return TextResponse(response_text=response_text, session_id=session_id, requires_confirmation=False)
    except Exception as e:
...
async def process_voice(request: VoiceRequest, background_tasks: BackgroundTasks):
    ...
    if not request.transcript or request.transcript.strip() == "":
        logger.warning("Empty transcript in process_voice")
    ...
    try:
        ...
        return VoiceResponse(response_text=response_text, session_id=session_id, requires_confirmation=False, meta={"tool_result": out.get("tool_result")})
